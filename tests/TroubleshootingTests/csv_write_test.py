...
# with open('lower_tier_only_300_iterations.csv', 'w') as sim_data_file:
...
for sim_num in range(0, 50):
    ...
    with open('lower_tier_only_300_iterations.csv', 'a') as sim_data_file:
        csv_writer = csv.writer(sim_data_file)

        for iteration_data in simulation_data:
            iteration_data[0] = sim_num + last_rep + 1
